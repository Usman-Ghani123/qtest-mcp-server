...
describe('deleteTestCycle', () => {
  ...
  it('GETs the cycle first, checks for children, then DELETEs it', async () => {
    const cycle = { id: 42, name: 'Sprint 3' }
    mockQtestFetch
      .mockResolvedValueOnce(cycle)  // GET /test-cycles/42
      .mockResolvedValueOnce([])     // GET children → none
    ...
    expect(mockQtestFetch).toHaveBeenNthCalledWith(
      2, expect.anything(), '1', '/test-cycles?parentId=42&parentType=test-cycle', 'GET'
    )
    expect(mockQtestFetch).toHaveBeenNthCalledWith(
      3, expect.anything(), '1', '/test-cycles/42', 'DELETE'
    )
  })
  ...
  it('recursively deletes child cycles before the parent', async () => {
    const parent = { id: 10, name: 'Parent' }
    const child = { id: 11, name: 'Child' }
    mockQtestFetch
      .mockResolvedValueOnce(parent)   // GET /test-cycles/10 (info)
    ...
    expect(mockQtestFetch).toHaveBeenNthCalledWith(
      4, expect.anything(), '1', '/test-cycles/11', 'DELETE'
    )
    expect(mockQtestFetch).toHaveBeenNthCalledWith(
      5, expect.anything(), '1', '/test-cycles/10', 'DELETE'
    )
  })
})
